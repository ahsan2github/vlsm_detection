...
for frameNo = frameVec 
    ...
    u_lvl_2 = uu(:,:,2);
    u_filtered = cuttoff_2D_multiF(u_lvl_2, 2 * pi * z_i / 2.0 / dz);    

	surf_pressure = pp(:,:,1);	
%   [N, binc] = hist(surf_pressure(:)./max(surf_pressure(:)),20);
% 	figure(); plot(binc, N./sum(N(:)));
	% calculation for high stress structures
	lowMomBin = u_lvl_2(:) < mean(u_lvl_2(:)); %+ 3.0 * std(shear_stress_field(:)); % gives a binary array 
	lowMomBin = reshape(lowMomBin, size(squeeze(u_lvl_2)));
    
	figure(); imagesc(lowMomBin); hold on; colormap gray;    

    avg_stress_slcth  = avg_stress_slcth  + data.shear_stress(:,:,heightVec);

	props = regionprops(lowMomBin, 'Centroid',...
    ...
    props = props(crit);
    count = length(props);
    lowMomMin = zeros(count,1);
    vpcnt = zeros(count,1);
    for indx = 1:count
        [box_lowMom, vpcnt, pxw, pyw] = getAvgBoxBotm2Blh(Nx, Ny, Nz, z_i, BLH, l_z,...
            bw_horz, bw_vert, props(indx).Centroid(1), props(indx).Centroid(2), data.shear_stress);
        lowMomMin(indx) = max(max(box_lowMom(:,:,2)));
        vpcnt(indx) = vpcnt;
    end
    ...
    while(~isempty(props))
        ...
        lowMomMin_temp = lowMomMin(conditionIsTrue);  
        [~, iindx] = min(lowMomMin_temp);
...
%% ============== plot to check sanity  ===================================
    ...
    for indx = 1:length(props)
        ...
        props(indx).Orientation, lowMomBin, lowMomBin);
...
%% =======================================================================       
    ...
    for counter = 1:length(props)
         ...
         bxw, byw,props(counter).Centroid(1), props(counter).Centroid(2), uu);
         uBox = uBox + box_u;
...
save('chnl_lowMomTrigger_smallerThanMean.mat','uBox','vBox','wBox','stressBox','pBox','omega1Box','divLambVecBox',...
    'omega2Box','omega3Box','lambda2Box','strucFound','vpcnt','avg_stress_slcth','heightVec','frameVec','-v7.3');
