...
%parentDir = '/glade/scratch/khanm/Ekman_Ugeo10_2048x2048x64_Lz1500';
%parentDir = '/scratch/ibrix/chpc_gen/u0699351/Ekman_Ugeo10_2048x2048x64_Lz1500';
outputDir = [parentDir '/output/'];
readinputs(parentDir);
BLH = 1433.00;
zo = 0.1;
u_star_in = u_star; clear u_star;
% get 3d stress array of sqrt(tau_13^2 + tau_23^2) for frames and save them as binary
for frameNo = frameVec 
    stresFieldName = sprintf('ek10_shearStress%4.4i.bin',frameNo);
...
for outerloop = 1:length(heightVec)   
    for slide = 1:length(envelope)
        ...
        for frameNo = frameVec 
            ...
            lowStreakBin_u = u_filtered(:) < 0.0; % gives a binary array 
            ...
            if(count >= 1) 
                ...
                msg =['Under given criteria no area was found, ' 'frame: ' ...
                        num2str(frameNo) '  StrucLength :' num2str(envelope(slide)) ...
...
save('ek10_areaFrac_stressFrac_hxx_xx_band.mat','areaFracVec_low','fluxFracVec_low','noRect_low',...
